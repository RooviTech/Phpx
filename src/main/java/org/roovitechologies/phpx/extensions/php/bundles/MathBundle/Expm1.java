 ...
 * @author AndreyPC
 */
public class Expm1 implements Function {
    
    @Override
    public Memory execute(Memory... args) {
        if(args.length != 1){
            PhpxApi.errorArgumentCountException("expm1", 1, args.length);
        }
        return NumberMemory.of(java.lang.Math.expm1(args[0].toDouble()));
    }
